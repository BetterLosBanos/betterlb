  ...
  try {
    ...
    if (!body.existing_document_id || !body.new_document || !body.action) {
      return Response.json(
        { error: 'Missing required fields: existing_document_id, new_document, action' },
        { status: 400 }
      );
    }
    ...
    ).bind(body.existing_document_id).first<{
      id: string;
      type: string;
      number: string;
      title: string;
            ...
            if (author.person_id && !author.is_new) {
              await env.BETTERLB_DB.prepare(
                `INSERT INTO document_authors (document_id, person_id, author_type)
                 VALUES (?1, ?2, ?3)`
              ).bind(existing.id, author.person_id, 'primary').run();
            }
          }
        }
      }
    ...
    if (body.action === 'merge') {
      ...
      if (updateFields.title && (!existing.title || existing.title.length < 20)) {
        updates.push(`title = ?${paramIndex++}`);
        params.push(body.new_document.title);
      }

      // Update session_id if existing doesn't have one
      if (updateFields.session_id && body.new_document.session_id && !existing.session_id) {
        updates.push(`session_id = ?${paramIndex++}`);
        params.push(body.new_document.session_id);
      }

      // Update moved_by if existing doesn't have one
      if (updateFields.moved_by && body.new_document.moved_by && !existing.moved_by) {
        updates.push(`moved_by = ?${paramIndex++}`);
        params.push(body.new_document.moved_by);
      }

      // Update seconded_by if existing doesn't have one
      if (updateFields.seconded_by && body.new_document.seconded_by && !existing.seconded_by) {
        updates.push(`seconded_by = ?${paramIndex++}`);
      ...
      if (updateFields.authors && body.new_document.authors && body.new_document.authors.length > 0) {
        // Get existing authors
        const existingAuthors = await env.BETTERLB_DB.prepare(
          `SELECT person_id FROM document_authors WHERE document_id = ?1`
        ).bind(existing.id).all<{ person_id: string }>();

        const existingAuthorIds = new Set(existingAuthors.results.map(a => a.person_id));

        // Add new authors that don't exist
        for (const author of body.new_document.authors) {
          if (author.person_id && !author.is_new && !existingAuthorIds.has(author.person_id)) {
            await env.BETTERLB_DB.prepare(
              `INSERT INTO document_authors (document_id, person_id, author_type)
               VALUES (?1, ?2, ?3)`
            ).bind(existing.id, author.person_id, 'primary').run();
    ...
    return Response.json({ error: 'Failed to resolve duplicate' }, { status: 500 });
  }
}
