...
export default function DepartmentDetail() {
  ...
  if (!dept)
    return (
      <div
        className='kapwa-text-disabled p-20 text-center font-bold tracking-widest uppercase'
        role='alert'
      >
        Office Not Found
      </div>
  ...
  return (
    <div className='animate-in fade-in space-y-8 pb-20 duration-500'>
      ...
      <header className='kapwa-text-inverse relative overflow-hidden rounded-2xl bg-slate-900 p-8 shadow-xl md:p-10'>
        ...
        <ClipboardList
          className='kapwa-text-inverse/5 pointer-events-none absolute right-[-20px] bottom-[-20px] h-64 w-64 -rotate-12'
          aria-hidden='true'
        />
      </header>
          ...
          <div className='bg-primary-50/50 kapwa-border-brand flex flex-col items-center gap-6 rounded-2xl border p-8 shadow-sm md:flex-row'>
            ...
            <div className='flex-1 text-center md:text-left'>
              ...
              {dept.department_head.email && (
                <div className='border-primary-100/50 kapwa-text-on-disabled mt-4 flex justify-center gap-4 border-t pt-4 text-sm md:justify-start'>
          ...
          <div className='grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3'>
            {associatedServices.map(service => (
              ...
              >
                <Card
                  hover
                  className='hover:kapwa-border-brand kapwa-border-weak h-full shadow-xs'
                >
                  <CardContent className='flex h-full items-center justify-between gap-3 p-4'>
                    <div className='flex min-w-0 items-center gap-3'>
                      <div className='bg-primary-50 kapwa-text-brand group-hover:kapwa-bg-brand-default kapwa-border-brand group-hover:kapwa-text-inverse shrink-0 rounded-lg border p-2 shadow-sm transition-colors'>
                      ...
                      <div className='min-w-0'>
                        ...
                        <p className='group-hover:kapwa-text-brand-bold kapwa-text-support text-sm leading-tight font-bold transition-colors'>
                          {service.service}
                        </p>
                      </div>
                    </div>
                    <ArrowRight className='group-hover:kapwa-text-brand h-4 w-4 shrink-0 text-slate-300 transition-all group-hover:translate-x-1' />
                  </CardContent>
                </Card>
              </Link>
            ))}
      ...
      <DetailSection title='Office Mandate' icon={InfoIcon}>
        <Card variant='slate' hover={false} className='bg-slate-50'>
          <CardContent className='p-6'>
            <p className='kapwa-text-on-disabled text-sm leading-relaxed md:text-base'>
