...
const SlideToggle = <T extends string>({
  ...
  return (
    <div className='kapwa-bg-hover relative inline-flex rounded-lg p-1'>
      {options.map(option => {
        ...
        return (
          <button
            ...
            className={`relative z-10 rounded-md px-3 py-1 text-xs font-medium transition-all duration-200 ${isActive ? 'text-slate-900' : 'hover:kapwa-text-support text-slate-500'} `}
          >
            {labels ? labels[option] : option}

            {/* The "Slide" Background Effect */}
            {isActive && (
              <span className='animate-in fade-in zoom-in-95 kapwa-border-weak kapwa-bg-surface absolute inset-0 -z-10 rounded-md border shadow-sm duration-200' />
            )}
          </button>
        );
      })}
  ...
  return (
    <div className='flex flex-col items-end gap-3 md:items-center'>
      ...
      <div className='flex flex-wrap items-center gap-3'>
        ...
        {/* View Mode Toggle */}
        <div className='flex items-center gap-2'>
          <PieChart className='kapwa-text-disabled h-4 w-4' />
          <SlideToggle
      ...
      {viewMode === 'quarter' && (
        <div className='animate-in fade-in slide-in-from-top-1 flex items-center gap-2 duration-200'>
          <span className='kapwa-text-disabled mr-1 text-xs font-semibold tracking-wider uppercase'>
            Period
          </span>
          <div className='kapwa-border-weak kapwa-bg-surface-raised flex rounded-lg border p-1'>
            {ALL_QUARTERS.map(q => {
                  ...
                  className={`relative rounded-md px-4 py-1.5 text-xs font-medium transition-all duration-200 ${
                    !isAvailable
                      ...
                      : isSelected
                        ...
                        : 'hover:kapwa-bg-surface text-slate-600 hover:shadow-sm' // Available & Hover
                  } `}
                >
                  {q}
                </button>
