...
export default function SubjectSearchAutocomplete({
  ...
  // Debounced search
  const searchSubjects = useCallback(async (searchQuery: string) => {
    ...
    try {
      ...
      if (isMockMode) {
        ...
        const response = await fetch(`/api/admin/subjects-search?q=${encodeURIComponent(searchQuery)}`);
        if (!response.ok) throw new Error('Failed to search subjects');

        const data = await response.json();
        const filtered = (data.subjects || []).filter((s: Subject) => !excludeNames.includes(s.name));
        setResults(filtered);
      }
      ...
      {showDropdown && (
        ...
        >
          {results.length > 0 ? (
            ...
            <div className="py-1">
              <div className="px-3 py-2 text-sm text-slate-500">No existing subjects found</div>
              <button
